...
const DEMOS = [
  { label: 'Sign in as Coach Morris', email: 'coach.morris@unityfcacademy.co.uk', password: 'DemoTPM2026!' },
  { label: 'Sign in as Club Admin (Alex)', email: 'admin.demo@unityfcacademy.co.uk', password: 'DemoTPM2026!' },
];
...
export function SignIn() {
  ...
  return (
    <div style={{ maxWidth: 420, margin: '0 auto', padding: 32, textAlign: 'center' }}>
      ...
      <h1 style={{ marginTop: 20 }}>Coach app</h1>
      <p style={{ color: 'var(--ink-muted)', marginTop: 8, fontSize: 13 }}>Pick a demo account or sign in.</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 20 }}>
        {DEMOS.map((d) => (
          <button key={d.email} className="btn btn-primary" onClick={() => doSignIn(d.email, d.password)} disabled={busy}>
            {busy ? 'Signing in…' : d.label}
          </button>
        ))}
      </div>
